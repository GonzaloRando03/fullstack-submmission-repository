...
function App() {
  ...
  function addPerson(event) {
    ...
    if (array.length === counter){
      ...
      if (window.confirm(newName + ' is already added to phonebook, change the number?')){
        let id
        array.map( person => {
  ...
  function deletePerson(person){
    if (window.confirm(`delete ${person.name} ?`)){
      setMessage(<div className='bad'>Information of {person.name} deleted from server</div>)
